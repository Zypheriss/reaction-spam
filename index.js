...
function patchClientUserSettingManager() {
    try {
        const managerPath = path.join(__dirname, 'node_modules', 'discord.js-selfbot-v13', 'src', 'managers', 'ClientUserSettingManager.js');
        ...
        for (const key in require.cache) {
            ...
            if (cacheKey === normalizedPath || cacheKey.endsWith('ClientUserSettingManager.js')) {
        ...
        if (ClientUserSettingManager && ClientUserSettingManager.prototype && ClientUserSettingManager.prototype._patch) {
            ...
            ClientUserSettingManager.prototype._patch = function(data) {
                if (data && 'friend_source_flags' in data) {
                    if (data.friend_source_flags === null || data.friend_source_flags === undefined) {
...
async function addReaction(client, guildId, channelId, messageId) {
    try {
        ...
        if (reactions.size > 1) {
            ...
            selectedReaction = reactionArray[Math.floor(Math.random() * reactionArray.length)];
        } else {
            selectedReaction = reactions.first();
        }
        await message.react(selectedReaction.emoji);
        const emojiName = selectedReaction.emoji.name || selectedReaction.emoji.identifier;
        console.log(`[${client.user.tag}] ✓ Reaction eklendi: ${emojiName}`);
        
    } catch (error) {
        console.error(`[${client.user?.tag || 'Bilinmeyen'}] ✗ Hata:`, error.message);
    }
}
async function main() {
    ...
    for (let i = 0; i < tokens.length; i++) {
        ...
        client.once('ready', async () => {
            console.log(`[${i + 1}/${tokens.length}] ${client.user.tag} - Giriş yapıldı`);
            await addReaction(client, messageData.guildId, messageData.channelId, messageData.messageId);
            setTimeout(() => {
                client.destroy();
            }, 2000);
        });
